...
class Contacto {
    ...
    public function agregar($nombre, $apellido, $email, $telefono) {
        // Consulta SQL con placeholders para evitar inyección SQL
        $sql = "INSERT INTO contactos (nombre, apellido, email, telefono) VALUES (?, ?, ?, ?)";
    ...
    public function actualizarContacto($id, $nombre, $apellido, $email, $telefono) {
        // Consulta SQL para actualizar un contacto según su ID
        $sql = "UPDATE contactos SET nombre = ?, apellido = ?, email = ?, telefono = ? WHERE id = ?";
        // Prepara la consulta
        $stmt = $this->conexion->prepare($sql);
